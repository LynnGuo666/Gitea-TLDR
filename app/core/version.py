...
VERSION_HISTORY = {
    ...
    "2.2.1": {
        "date": "2026-05-13",
        "changes": [
            "增强：Forge review 提示词全面升级——补充基本规则（新版本行号、行号跨度 ≤100）、Step 5.5 自我验证二次审查、Mermaid 完整指南（变更规模决图数量、业务流/技术流区分、正反例对比、完整颜色表）",
            "增强：Forge review 提示词新增全套禁止评论类型约束（纯描述、夸赞、变更叙述、不确定措辞、格式风格）、跳过文件清单、UI 代码处理规则、上下文优先原则",
            "增强：Forge issue 提示词完整融入证据驱动方法论——假设状态三分类（已证实/已推翻/无法确认）、证据门硬约束、假设格式规范及四条具体示例",
            "修复：移除 webhook_handler 中冗余的 _build_inline_section 文本兜底，行内批注统一通过 Gitea 正式 review 接口发送",
        ],
    },
    "2.2.0": {
        "date": "2026-05-13",
    ...
    "1.23.4": {
        "date": "2026-04-09",
        "changes": [
            "优化：侧边栏后端不可达或 OAuth 未配置时，左下角用户区域改为与已登录状态一致的 Dropdown 样式，展示版本号与更新日志入口，不再显示误导性提示文字",
            "修复：前端仅启动时，Layout 不再将后端网络错误误判为 OAuth 未配置，主内容区直接渲染页面内容而非报错页",
            "维护：同步更新后端与前端版本号到 1.23.4",
        ],
    },
    "1.23.3": {
        "date": "2026-04-08",
    ...
    "1.21.3": {
        "date": "2026-02-21",
        "changes": [
            "新增：usage_stats 增加 user_id 字段，审查用量记录按触发人归类到用户维度",
            "新增：/api/stats 在登录态下默认按当前用户聚合与查询用量数据",
            "修复：历史用量数据迁移时自动回填到 id=1 的用户（若不存在则自动创建）",
            "维护：同步更新前后端版本号到 1.21.3",
        ],
    },
    "1.21.2": {
        "date": "2026-02-21",
        "changes": [
            "修复：收紧管理后台用户更新权限，非 super_admin 仅允许修改自己的基础信息，阻止管理员自提权为 super_admin 或自扩权 permissions",
            "修复：用户创建/更新接口增加角色与权限一致性校验，仅 admin 角色可配置 permissions，角色降级时自动清理遗留权限",
            "修复：删除、停用或降级用户时增加“至少保留一个启用中的 super_admin”保护，避免权限系统锁死",
            "修复：ensure_initial_admin 支持提升已存在用户为 super_admin，避免初始管理员恢复因唯一键冲突失效",
            "优化：前端用户管理页按角色精细化禁用操作入口，非 super_admin 仅可编辑自己并避免无效请求",
            "维护：同步更新前后端版本号到 1.21.2",
        ],
    },
    "1.21.1": {
        "date": "2026-02-21",
    ...
    "1.20.1": {
        "date": "2026-02-21",
        "changes": [
            "调整：用户中心移除“服务状态”区块，页面聚焦账号与使用统计信息",
            "新增：管理后台首页补充“服务状态”区块，集中展示 Bot 用户名、Debug 模式与 OAuth 启用状态",
            "维护：同步更新前后端版本号到 1.20.1",
        ],
    },
    "1.20.0": {
        "date": "2026-02-20",
        "changes": [
            "新增：管理后台用户管理页（/admin/users），支持管理员列表查看、创建、编辑、删除与启停",
            "新增：管理后台首页快捷入口“用户管理”，支持从 Dashboard 直接进入用户权限管理流程",
            "优化：管理员用户 API 响应增加 permissions 字段，前端可直接读取结构化权限配置",
            "维护：同步更新前后端版本号到 1.20.0",
        ],
    },
    "1.19.7": {
        "date": "2026-02-21",
    ...
    "1.11.0": {
        "date": "2026-01-21",
        "changes": [
            "修复：Webhook 启用状态下 Toggle 开关显示错误，滑块位置与挤压问题",
            "修复：首页所有仓库默认显示为已启用，后端无记录时 is_active 默认值改为 False",
            "修复：监听事件文本过长导致开关按钮被挤出容器，Flexbox 布局自适应",
        ],
    },
    "1.10.0": {
        "date": "2026-01-21",
    ...
    "1.8.0": {
        "date": "2026-01-20",
        "changes": [
            "新增：管理后台系统，支持系统管理和监控（Dashboard 看板、用户管理、全局配置、Webhook 日志）",
            "新增：基于数据库的管理员列表与权限配置（super_admin/admin 角色）",
            "新增：AdminSettings 全局配置表，分类存储系统配置",
            "新增：WebhookLog 日志表，记录所有请求的完整信息和处理状态",
            "新增：权限中间件 admin_auth.py，提供 admin_required() 装饰器",
        ],
    },
    "1.7.0": {
        "date": "2026-01-20",
