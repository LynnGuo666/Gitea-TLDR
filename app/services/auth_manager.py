...
class AuthManager:
    ...
    def __init__(self) -> None:
        self.enabled = bool(
            ...
            and settings.oauth_redirect_url
        )
        self._state_store: Dict[str, float] = {}
        self._sessions: Dict[str, SessionData] = {}
        self._lock = Lock()
        self._authorize_endpoint = f"{settings.gitea_url.rstrip('/')}/login/oauth/authorize"
        self._token_endpoint = f"{settings.gitea_url.rstrip('/')}/login/oauth/access_token"
    ...
    async def handle_callback(self, code: str, state: str, response: Response) -> Response:
        if not self.enabled:
            raise HTTPException(status_code=400, detail="OAuth 尚未配置")
    ...
    def _attach_cookie(self, response: Response, session_id: str, expires_in: int) -> None:
        response.set_cookie(
