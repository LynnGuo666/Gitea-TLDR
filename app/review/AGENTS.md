...
WebhookHandler.handle()
    ...
    ├── handle_comment_event() → 解析命令 → /review → ReviewEngine / /issue → IssueAnalysisService / /tag-review → perform_tag_review
    ├── handle_issue() → IssueAnalysisService.analyze_issue()
    └── handle_create() (tag 创建) → perform_tag_review(上一个 tag → 新 tag)
    └── 审计日志 → AuditService
```
...
- 所有 webhook 入口需验证签名（`WEBHOOK_SECRET` 已配置时），`POST /webhook` 校验 `X-Gitea-Signature`（HMAC-SHA256 of raw body）。
...
- tag 区间审查（`perform_tag_review`）复用 `_perform_review` 骨架，但 diff 来自 `GiteaClient.compare_tags`、克隆用 `RepoManager.clone_for_compare`、结果不回写 Gitea 评论（tag 无评论载体），只设置 commit status + 飞书推送；幂等 key 为 `(repo, to_tag, head_sha)`。
- 飞书推送（`FeishuNotifier`）作为非致命旁路接入 `_perform_review` / `analyze_issue` / `perform_tag_review` 末尾，失败只 log.warning，不抛。
