...
- **Webhook 保存权限**: 仓库 Webhook 状态查询与保存改为使用当前登录用户的 OAuth token，并在服务端校验仓库 admin 权限，修复误用 Bot PAT / `GITEA_TOKEN` 导致配置失败的问题
- **Webhook 目标地址**: 默认目标地址改为根据请求上下文生成完整 `/webhook` URL，拒绝相对 URL，避免向 Gitea 保存不可用的 `"/webhook"`
- **Webhook 事件配置**: 后端现在尊重前端选择的触发事件，并对非法或空事件列表返回明确 400 错误
- **错误提示**: 仓库配置页保存 Webhook 时检查 HTTP 状态并展示后端错误详情，不再静默吞掉 401/403/502 等失败

### 测试 (Tests)

- **Webhook 回归测试**: 新增登录态、仓库 admin 权限、用户 OAuth client 调用、默认绝对 URL 与相对 URL 拒绝的后端测试覆盖
...
- **只读仓库可点击**: 修复只读仓库在列表页无法点击跳转的问题，现所有仓库均可点击进入详情页；管理配置入口维持 admin 权限校验

### 维护 (Maintenance)

- **版本一致性**: 同步更新后端与前端版本号到 `2.0.5`
...
- **审查记录补充 Token 用量**: 审查记录列表与详情接口现返回每次审查对应的 token 用量字段，包括输入、输出、缓存写入、缓存读取与 `total_tokens`
- **记录页面展示 Tokens**: 前端“审查记录”和“管理后台审查历史”页面新增 Tokens 展示，列表可直接查看总 tokens，展开详情可查看输入、输出与缓存 token 明细

### 测试 (Testing)

- **审查历史接口回归测试**: 新增覆盖 review sessions 序列化 token 用量字段的回归测试，避免后续接口精简时再次丢失

### 维护 (Maintenance)

- **版本一致性**: 同步更新后端与前端版本号到 `1.26.6`

## [1.26.5] - 2026-04-16

### 增强 (Improved)

- **Anthropic 自动重试**: Forge 的 Anthropic API 客户端新增自动重试机制，对 `429`、`5xx`（含 `529 overloaded_error`）及瞬时网络异常按指数退避重试 3 次
- **重试策略更稳健**: 优先尊重 `Retry-After` 响应头，`401` 等明确鉴权错误继续快速失败，避免无意义重复请求

### 测试 (Testing)

- **Forge API 客户端回归测试**: 新增测试覆盖 `529` 连续失败后重试成功与 `401` 不重试两类场景

### 维护 (Maintenance)

- **版本一致性**: 同步更新后端与前端版本号到 `1.26.5`
...
- **侧边栏用户区域**: 后端不可达或 OAuth 未配置时，左下角用户区域现在与已登录状态保持一致的 Dropdown 样式，点击可查看版本号与更新日志，副标题分别显示"后端未连接"或"请配置 OAuth"
- **Agentation 工具栏**: 将 `<Agentation />` 移至 `<Layout>` 外层，避免因 Layout 的 CSS 遮挡导致悬浮按钮不显示

### 修复 (Fixed)

- **前端单独启动**: 后端不可达时，Layout 不再将网络错误误判为"OAuth 未配置"并阻断页面渲染，主内容区现在直接渲染页面内容

### 维护 (Maintenance)

- **版本一致性**: 同步更新后端与前端版本号到 `1.23.4`
...
- **Claude 代理响应语义**: 重构 `UsageCapturingProxy` 转发逻辑，非流式响应补齐 `Content-Length`，SSE 改为原始字节透传，修复代理路径下 Claude CLI 容易卡死并最终 300 秒超时的问题
- **多请求连接复用**: `UsageCapturingProxy` 现支持同一连接上的连续非流式请求，避免出现“只有第一个请求被转发，后续请求消失”的现象
- **代理诊断信息**: 为 usage 代理补充 `last_error` 与可选 debug 日志，超时错误会附带代理异常线索，便于观察真实故障点
- **审查记录页反馈**: 审查记录页对进行中的任务启用自动刷新，并明确展示“审查进行中”的等待提示，避免详情区域看起来像空白

### 测试 (Testing)

- **Usage Proxy 回归测试**: 新增代理回归测试，覆盖多请求连接复用、SSE 原样透传、usage 捕获累计与代理开关行为

### 维护 (Maintenance)

- **版本一致性**: 同步更新后端与前端版本号到 `1.22.3`
...
- **用量用户归类**: `usage_stats` 新增 `user_id` 字段，审查用量落库按触发人（Webhook sender / 评论触发者）归类到用户维度
- **用户视角统计**: `GET /api/stats` 在登录态下默认按当前用户过滤，`/usage` 与用户中心的“使用统计”改为展示当前用户数据

### 修复 (Fixed)

- **历史数据回填**: Alembic 迁移会将历史未归类的用量记录统一回填到 `users.id = 1` 的账号；若该账号不存在则自动创建后再回填

### 维护 (Maintenance)

- **版本一致性**: 同步更新后端与前端版本号到 `1.21.3`

## [1.21.2] - 2026-02-21

### 修复 (Fixed)

- **用户权限越权修复**: 收紧 `PUT /api/admin/users/{username}` 权限模型，非 `super_admin` 仅允许修改自己的基础信息（如邮箱），禁止修改 `role` / `permissions` / `is_active`
- **角色与权限一致性**: 用户创建/更新接口增加校验，仅 `admin` 角色可配置 `permissions`；角色变更为非 `admin` 时自动清理遗留细粒度权限
- **超级管理员保底保护**: 删除、停用或降级用户时新增“至少保留一个启用中的 `super_admin`”保护，避免系统进入无超级管理员状态
- **初始管理员恢复**: `ensure_initial_admin` 支持在用户已存在时直接提升为 `super_admin` 并启用，修复唯一键冲突导致的恢复失败

### 优化 (Improved)

- **前端权限交互对齐**: `/admin/users` 页面根据角色禁用不允许的操作入口，非 `super_admin` 用户仅可编辑自己并仅提交允许字段，减少无效 403 请求

### 维护 (Maintenance)

- **版本一致性**: 同步更新后端与前端版本号到 `1.21.2`
...
- **用户中心信息聚焦**: `/settings` 页面移除“服务状态”区块，保留账号信息与使用统计，减少非个人维度的配置展示
- **管理后台职责收敛**: 将服务状态集中到 `/admin` 首页展示（Bot 用户名、Debug 模式、OAuth 登录状态），便于管理员统一查看
- **版本一致性**: 同步更新后端与前端版本号到 `1.20.1`

## [1.20.0] - 2026-02-20

### 新增功能 (Added)

- **管理后台用户管理页**: 新增 `/admin/users` 页面，支持管理员列表查看、创建、编辑、删除与账号启停
- **Dashboard 快捷入口**: 管理后台首页新增“用户管理”入口卡片，可直接进入账户与权限管理

### 优化 (Improved)

- **管理员权限数据透出**: `GET /api/admin/users` 相关响应新增 `permissions` 字段，并返回已解析的结构化权限配置，便于前端直接渲染和编辑
- **版本一致性**: 同步更新后端与前端版本号到 `1.20.0`

### 技术改进 (Technical)

- **Compose 配置清理**: 清理 `docker-compose.yml` 中 healthcheck `start_period` 的冗余行内注释，保持部署配置简洁
