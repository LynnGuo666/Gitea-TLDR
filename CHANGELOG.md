...
- **审查审计字段**: 审查会话新增并持久化 `provider_name`、`model_name`、`config_source`，便于追踪每次审查的引擎与配置来源
- **个人审查历史接口**: 新增 `GET /api/my/reviews`，按当前用户可访问仓库范围返回审查记录（无 OAuth 配置时回退为全量）
- **个人审查记录页面**: 新增 `/reviews` 页面，支持筛选成功/失败、按行展开查看摘要与行级评论

### 优化 (Improved)

- **GUI 审查方向生效链路**: 修复仓库页面配置只保存不生效的问题，补齐 webhook/命令触发时的配置回退优先级
- **管理后台审查历史页**: `admin/reviews` 从占位页升级为完整列表，支持筛选、分页、展开查看详情
- **导航入口补齐**: 侧边栏新增“审查记录”入口，统一访问个人审查历史
