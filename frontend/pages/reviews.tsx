...
export default function ReviewsPage() {
  ...
  useEffect(() => {
    apiFetch('/api/v2/runs?kind=review&limit=50&offset=0')
      .then((res) => res.json())
        ...
        <div className="mt-4 grid gap-3">
          {runs.map((run) => (
            <div key={run.id} className="border-b border-divider py-3">
              ...
              <div className="text-sm text-default-500">{run.repo_full_name || `repo ${run.repository_id}`} · {run.status} · {run.effective_engine || 'engine unknown'}</div>
