...
export default function AdminIssuesPage() {
  const { data, loading, error, refresh } = useApiFetch<{ runs: AnalysisRunSummary[] }>(
    '/api/v2/runs?kind=issue&limit=100&offset=0'
  );
  ...
  return (
    <>
      ...
      <div className="max-w-[1100px] mx-auto">
        <PageHeader title="Issue Runs (Admin)" subtitle="全部 Issue 分析运行记录" />
            ...
            runs.map((run) => (
              <div key={run.id} className="border-b border-divider py-3">
                ...
                <div className="text-sm text-default-500">
                  {run.repo_full_name || `repo ${run.repository_id}`} · {run.status} · {run.effective_model || 'model unknown'}
